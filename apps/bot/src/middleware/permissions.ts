...
export async function canUseFeature(
  ...
  // Fetch all relevant configs for this server and featureKey in a single query
  const configs = await prisma.featureConfig.findMany({
    where: {
      ...
      OR: [
        ...
        { targetType: TargetType.SERVER, targetId: serverId },
      ],
    },
  });
  ...
  // 3. Server specificity (Global default for the server)
